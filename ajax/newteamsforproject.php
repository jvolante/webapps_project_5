...
if (isset($_POST["teams"]) && isset($_POST["project"])) {
  ...
  $conn->query("DELETE FROM $dbname.jk_team WHERE project='$currentProject';") or die("Error on delete");

  $teams = json_decode($_POST["teams"]) or die("JSON syntax error");

  foreach ($teams as $teamid => $members) {
    foreach ($members as $key => $name) {
      $conn->query("INSERT INTO $dbname.jk_team VALUES ($teamid, '$currentProject', '$name');");
    }
  }
  echo 'success';
} else {
